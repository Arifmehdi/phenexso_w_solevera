                ...
                <div class="sidebar-content">
                    <div class="sticky-sidebar" data-sticky-options="{'top': 10}">
                        <div class="filter-actions mb-4">
                            ...
                            <a href="{{ route('shop') }}" class="filter-clean">Clean All</a>
                        </div>
                        <div class="widget widget-collapsible">
                            <h3 class="widget-title">All Categories</h3>
                            <ul class="widget-body filter-items search-ul">
                                <li class="{{ !request()->route('slug') ? 'active' : '' }}"><a href="{{ route('shop') }}">All ({{ $total_products }})</a></li>
                                @foreach ($allRootCategories as $rootCategory)
                                    <li class="{{ request()->route('slug') == $rootCategory->slug ? 'active' : '' }}">
                                        ...
                                        <ul style="{{ (request()->route('slug') == $rootCategory->slug || $rootCategory->children->contains('slug', request()->route('slug'))) ? 'display: block' : '' }}">
                                            @foreach($rootCategory->children as $child)
                                            <li class="{{ request()->route('slug') == $child->slug ? 'active' : '' }}"><a href="{{ route('productCategory', $child->slug) }}">{{ $child->name_en }} ({{ $child->products()->count() }})</a></li>
                                            @endforeach
                                        </ul>
                                        @endif
                                    </li>
